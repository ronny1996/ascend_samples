...
# pool = nn.AvgPool2D(kernel_size=(2, 2), stride=2, padding=0, data_format="NCHW")
# y_var = pool(x_var)
# # print(y_var.numpy())
# s = y_var.sum()
# s.backward()
...
# x_np = np.random.random([1, 1, 5, 5]).astype(np.float32)
x_np = np.random.random([1, 1, 5, 5]).astype(np.float32)
x_var = paddle.to_tensor(x_np, stop_gradient=False)
pool = nn.AvgPool2D(kernel_size=(2, 2), stride=2, padding=2, data_format="NCHW")
y_var = pool(x_var)
s = y_var.sum()
s.backward()
print(x_var.grad.numpy().squeeze())
